...
case "${ACTION}" in
  ...
  status)
    ...
    kubectl get pods -n "${K8S_NAMESPACE}" -l "app.kubernetes.io/name=opentelemetry-collector" -o wide 2>/dev/null || true
    kubectl get svc -n "${K8S_NAMESPACE}" -l "app.kubernetes.io/name=opentelemetry-collector" 2>/dev/null || true
    ;;
  logs)
    log_info "Fetching logs for ${SERVICE_KEY}..."
    kubectl logs -n "${K8S_NAMESPACE}" -l "app.kubernetes.io/name=opentelemetry-collector" --all-containers=true "$@"
    ;;
  health)
  ...
  port-forward)
    ...
    kubectl port-forward -n "${K8S_NAMESPACE}" "svc/llmobs-opentelemetry-collector" "${local_port}:${DEFAULT_PORT}" --address="0.0.0.0"
    ;;
  *)
