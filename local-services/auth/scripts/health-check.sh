...
echo -e "${BLUE}====================================================${NC}"
echo -e "${BOLD} AUTH SERVICE STACK HEALTH DIAGNOSTIC               ${NC}"
echo -e "${BLUE}====================================================${NC}"
...
test_alloydb_crud() {
  ...
  if [ -f "$AUTH_DIR/.env" ]; then
    db_user=$(grep -E "^ALLOYDB_USER=" "$AUTH_DIR/.env" 2>/dev/null | cut -d= -f2 || echo "admin")
    db_pw=$(grep -E "^ALLOYDB_PASSWORD=" "$AUTH_DIR/.env" 2>/dev/null | cut -d= -f2 || echo "llmobs_s3cret_2026")
    db_name=$(grep -E "^ALLOYDB_DB=" "$AUTH_DIR/.env" 2>/dev/null | cut -d= -f2 || echo "llm_observability")
  fi

  local sql="CREATE TABLE ${tbl} (id INT PRIMARY KEY, payload TEXT); INSERT INTO ${tbl} VALUES (1, 'auth_db_ok'); SELECT payload FROM ${tbl}; DROP TABLE ${tbl};"
  ...
  while [ $attempt -lt $max_attempts ]; do
    res=$(docker exec -e PGPASSWORD="$db_pw" -i auth-service-db psql -U "$db_user" -d "$db_name" -c "$sql" 2>/dev/null || echo "")
    if echo "$res" | grep -q "auth_db_ok"; then
      break
    fi
    attempt=$((attempt + 1))
    if [ $attempt -ge $max_attempts ]; then
  ...
  if echo "$res" | grep -q "auth_db_ok"; then
    echo -e "  ${GREEN}[PASS]${NC} ${BOLD}Auth Database (PostgreSQL/AlloyDB) CRUD Verification${NC} -> Table create/insert/select/drop OK"
    PASSED_CHECKS=$((PASSED_CHECKS + 1))
  else
    echo -e "  ${RED}[FAIL]${NC} ${BOLD}Auth Database (PostgreSQL/AlloyDB) CRUD Verification${NC} -> Query execution failed"
  fi
}

test_redis_crud() {
  TOTAL_CHECKS=$((TOTAL_CHECKS + 1))
  local key="health:auth:$(date +%s)"
  local val=""
  local attempt=0
  local max_attempts=8
  local base_delay=1
  ...
  while [ $attempt -lt $max_attempts ]; do
    docker exec -i llmobs-redis-ledger redis-cli -a llmobs_redis_s3cret_2026 SET "$key" "redis_ok" >/dev/null 2>&1 || true
    val=$(docker exec -i llmobs-redis-ledger redis-cli -a llmobs_redis_s3cret_2026 GET "$key" 2>/dev/null || echo "")
    docker exec -i llmobs-redis-ledger redis-cli -a llmobs_redis_s3cret_2026 DEL "$key" >/dev/null 2>&1 || true
  ...
  if echo "$val" | grep -q "redis_ok"; then
    echo -e "  ${GREEN}[PASS]${NC} ${BOLD}Redis Ledger CRUD Verification${NC} -> Key set/get/del verification OK"
    PASSED_CHECKS=$((PASSED_CHECKS + 1))
  else
    echo -e "  ${RED}[FAIL]${NC} ${BOLD}Redis Ledger CRUD Verification${NC} -> Key-value write failed"
  fi
}

echo -e "\n${YELLOW}1. Container Process & Docker Health Status:${NC}"
check_container_status "auth-service-db" "Database (PostgreSQL 15+ / AlloyDB Omni)"
check_container_status "llmobs-redis-ledger" "Redis Ledger"
...
check_container_status "llmobs-service-registry" "Service Registry API"

echo -e "\n${YELLOW}2. Port Accessibility & Endpoint Verification:${NC}"
check_tcp "Database Port" "5432"
check_tcp "Redis Ledger Port" "6379"
check_tcp "Kafka Event Broker Port" "9092"
check_tcp "OTel Collector HTTP" "4318"
check_tcp "OTel Collector gRPC" "4317"
check_http "Service Registry API" "http://localhost:31426/health" "200"
