...
COMPOSE_FILE="$NOTIFICATION_DIR/docker-compose.yml"
...
ensure_env() {
  if [ ! -f "$NOTIFICATION_DIR/.env" ] && [ -f "$NOTIFICATION_DIR/.env.example" ]; then
    echo -e "${BLUE}Copying .env.example to .env...${NC}"
    cp "$NOTIFICATION_DIR/.env.example" "$NOTIFICATION_DIR/.env"
  fi
}

ensure_data_storage() {
  local data_dir="${LLMOBS_DATA_DIR:-$NOTIFICATION_DIR/data}"
