resource "google_compute_address" "storage_ip" {
  name   = "${var.instance_name}-ip"
  region = var.gcp_region
}

resource "google_compute_instance" "storage_node" {
  name         = var.instance_name
  machine_type = var.machine_type
  zone         = var.gcp_zone
  tags         = ["llmobs-storage-node"]
  ...
  network_interface {
    ...
    access_config {
      nat_ip = google_compute_address.storage_ip.address
    }
  }
